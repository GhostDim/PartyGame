...
class GameStateManager {
  ...
  createAdminToken() {
    ...
    this.adminTokens.set(token, { playerId: null });
    return token;
  }
  ...
  linkAdminPlayer(token, playerId) {
    const row = this.adminTokens.get(token);
    if (!row) return;
  ...
  remove(playerId) {
    ...
    for (const row of this.adminTokens.values()) {
      if (row.playerId === playerId) row.playerId = null;
    }
  ...
  kick(playerId) {
    const player = this.players.get(playerId);
    if (!player) return { ok: false, error: 'Игрок не найден' };
    if (player.isHost) return { ok: false, error: 'Ведущего нельзя удалить' };
